...
pub fn apply_grain_8bpc_mt(
    ...
    // Correctness-first safe path: keep film grain application single-threaded
    // for now instead of sharing raw destination pointers across worker jobs.
    // This removes the Send/Sync raw-pointer wrapper and the slice
    // reconstruction from raw parts. A future optimization can reintroduce
    // parallelism with `split_at_mut` row-band slices.
    for row in 0..rows {
        apply_grain_row_8bpc(
            dst_y, dst_u, dst_v, src_y, src_u, src_v, y_stride, uv_stride, fgd, &grain_lut,
            &scaling, w, h, row, seed, ss_x, ss_y,
